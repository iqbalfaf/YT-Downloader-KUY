...
check_dependencies() {
    ...
    if ! command -v yt-dlp >/dev/null 2>&1; then
        ...
        echo "Linux: pip install yt-dlp  ATAU  sudo apt install yt-dlp"
...
while true; do
    clear
    echo -e "${GREEN}=========================================${NC}"
    echo -e "${GREEN}       YT DOWNLOAD iBay KUY (Linux)${NC}"
    echo -e "${GREEN}=========================================${NC}"
    ...
    case "$main_choice" in
        ...
        4) # MENU COOKIES
            while true; do
                ...
                echo "3. Ambil Cookies dari Brave Browser"
                echo "4. Gunakan file cookies.txt (di folder script)"
                echo "5. Nonaktifkan Cookies"
                echo "0. Kembali ke Menu Utama"
                echo -e "${GREEN}=========================================${NC}"
                read -p "Pilih opsi (1-5, 0 untuk kembali): " cookie_choice
        ...
        5) # UPDATE YTDLP
            while true; do
                ...
                if [ "$update_choice" == "0" ]; then
                    ...
                    yt-dlp -U || echo -e "\nJika gagal, coba: pip install --upgrade yt-dlp atau sudo apt install --only-upgrade yt-dlp"
                    echo ""
